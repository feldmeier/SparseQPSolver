...
void HMatrix::compute_mat_mat(HMatrix& mat, HMatrix& res) {
	assert(numCol == mat.numRow);
	int resRow = numRow;
	int resCol = mat.numCol;

	vector<HVector> rows;

	for(int i = 0; i < numRow; i++) {
		//compute vec_mat product of row i of (this) and mat
		//get row i of (this)
		int idxStart = ARstart[i];
		int idxEnd = ARstart[i+1];
		int entries = idxEnd-idxStart;
		if(entries > 0) {
			int* index = new int[entries];
			double* value = new double[entries];
			for(int j = 0; j < entries; j++) {
				index[j] = ARindex[idxStart + j];
				value[j] = ARvalue[idxStart + j];
			}
			HVector row;
			row.setup(entries, index, value);

			//compute  vec_mat product
			HVector resultRow;
			resultRow.setup(resCol);
			mat.compute_vec_mat(row, resultRow);
			rows.push_back(resultRow);
		}
	}
	//put all rows together to a HMatrix structure
	//count number of vector elements
	int elecount = 0;
	int* resstart = new int[numCol + 1];
	for(int i = 0; i < rows.size(); i++) {
		elecount += rows[i].count;
	}
	int* resindex = new int[elecount];
	double* resvalue = new double[elecount];
	int counter = 0;
	for(int i = 0; i < mat.numCol; i++) {
		resstart[i] = counter;
		//find ith element of every rowvector
		for(int j = 0; j < rows.size(); j++) {
			//find ith element of this rowvector
			HVector row = rows[j];
			for(int k = 0; k < row.count; k++) {
				if(row.index[k] == i) {
					resindex[counter] = j;
					resvalue[counter] = row.array[k];
					counter++;
					break;
				}
			}
	...
	resstart[mat.numCol] = counter;
	res.setup_lgBs(mat.numCol, numRow, resstart, resindex, resvalue);
	//res.rp_mtx();
}
...
void HMatrix::compute_vec_mat(HVector& vec, HVector& res) {
	int* resindex = new int[numCol];
	double* resvalue = new double[numCol];

	for(int i = 0; i < numCol; i++) {
		resvalue[i] = 0;
		resindex[i] = i;
		for(int j = Astart[i]; j < Astart[i+1]; j++) {
			//see if the vector has a corresponding nonzero entry
			for(int k = 0; k < vec.count; k++) {
				if(vec.index[k] == Aindex[j]) {
					resvalue[i] += vec.array[k] * Avalue[j];
					break;
				}
			}
		}
	}
	res.setup(numCol, resindex, resvalue);
}
...
void HMatrix::compute_matB_vec(const double *vec, const int *base,
    ...
    int *resultIndex = &result->index[0];
    double *resultArray = &result->array[0];
    //TODO: verify that numCol is indeed correct and numRow wrong
    for (int i = 0; i < numCol; i++) {
        int iCol = base[i];
        double value = vec[i];
        if (fabs(value) > HSOL_CONST_TINY) {
